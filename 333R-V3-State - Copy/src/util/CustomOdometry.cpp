...
std::valarray<double> CustomOdometry::getSensorVals() // returns new sensor values
{
    return {meS.get(), (((def::mtr_dt_right_front.getPosition())* def::DRIVE_DEG_TO_IN) 
                + 
                ((def::mtr_dt_right_back.getPosition())* def::DRIVE_DEG_TO_IN))/2,
            ((isinf(mimu1.get_rotation()) ? 0 : mimu1.get_rotation()) +
             (isinf(mimu2.get_rotation()) ? 0 : mimu2.get_rotation())) *
                3.14 / 180 / 2, 
                (((def::mtr_dt_left_front.getPosition())* def::DRIVE_DEG_TO_IN) 
                + 
                ((def::mtr_dt_left_back.getPosition())* def::DRIVE_DEG_TO_IN))/2}; // /-360
}
...
{
    const double df =
        ivalsDiff[1] / 360; // stores the change of all tracking wheels in inches
    const double ds = ivalsDiff[0] * mcircIn / 360;
    const double df2 = ivalsDiff[3] / 360;
