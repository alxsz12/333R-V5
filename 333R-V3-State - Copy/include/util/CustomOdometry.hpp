...
class CustomOdometry // class that organizes the sensors, calculations, and state variables for
                     // odometry
{
  ...
  /* -------------------- Sensor References -------------------- */
  static Motor &mlf;  // left tracking wheel encoder
  static Motor &mrf;  // left tracking wheel encoder
  static Motor &mlb;  // left tracking wheel encoder
  static Motor &mrb;  // left tracking wheel encoder
  ...
  /* ------------------------ Variables ------------------------ */
  static OdomState mstateInitial; // starting position of the robot
...
public:
  static pros::Mutex odom_mutex; // mutex for protecting position variables
  ...
  static void
  setStateInitial(const OdomState &istate);      // sets the initial and current states of the robot
  ...
  static OdomState mathStep(std::valarray<double> ivalsDiff); // does one iteration of odometry
                                                              // math, given sensor changes
};

namespace def
{
  extern CustomOdometry customOdom; // declares the customOdom object as extern, to make sure it only
                                    // gets constructed once
}
